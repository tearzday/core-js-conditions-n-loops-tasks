...
function getMaxNumber(a, b, c) {
  const maxFirstСouple = a > b ? a : b;
  return maxFirstСouple > c ? maxFirstСouple : c;
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    sumLeftItems += arr[i];
    if (sumLeftItems === sumItems - sumLeftItems - arr[i + 1]) {
      balanceIndex = i + 1;
    }
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < resultMatrix.length; i += 1) {
    let rowPositionItem = 0;
    for (let j = resultMatrix.length - 1; j >= 0; j -= 1) {
      matrixRow[rowPositionItem] = resultMatrix[j][i];
      rowPositionItem += 1;
    }
